"""
This module implements a custom regression random forest for tabular data,
supporting sample weights for robust regression.
"""
...
class BoostedRandomForest:
    ...
    This algorithm builds an ensemble of WeightedRegressionTree models. Each tree is
    ...
    def fit(
        ...
        for t in range(self.n_trees):
            ...
            w_sample = w[bootstrap_indices]

            # Select a random subset of features
            feature_indices = self._rng.choice(
                n_features,
                self.n_features_sample,
                replace=False)
            ...
            tree.fit(X_sample[:, feature_indices], y_sample, w=w_sample)
            self.trees.append(tree)

            prediction = tree.predict(X[:, feature_indices])
            error = np.abs(y - prediction)
            w = 1.0 + error / (error.max() + 1e-5)
            w = w / w.sum()
